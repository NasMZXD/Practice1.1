...
class _DateTimeScreenState extends State<DateTimeScreen> {
  ...
  Future<void> _markAsCompleted(int id) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Статус заявки обновлен на "выполнено"')),
      );
    } catch (e) {
  ...
  Widget _buildRepairRequestsTable() {
    return repairRequests.isEmpty
        ? Center(child: Text('Нет данных для отображения заявок на ремонт'))
  ...
  Widget _buildPaintingRequestsTable() {
    return paintingRequests.isEmpty
        ? Center(child: Text('Нет данных для отображения заявок на покраску'))
        : SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: DataTable(
              columns: [
  ...
  Future<void> _markPaintingAsCompleted(int id) async {
    try {
      ...
      await _fetchData(); // Обновляем данные после изменения статуса
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Статус заявки на покраску обновлен на "выполнено"')),
      );
    } catch (e) {
